...
export default class SCMIntegrations {
     ...
     */
    find(integrationID) {
        return unpackData(this._client.authApiClient().get(`/scm_integrations/${integrationID}`), "scm_integration");
    }
    /**
     ...
     */
    all() {
        return unpackData(this._client.authApiClient().get("/scm_integrations"), "scm_integrations");
    }
    /**
    ...
    create(opts) {
        return unpackData(this._client
            .authApiClient()
            .post("/scm_integrations", { scm_integration: opts }), "scm_integration");
    }
    /**
    ...
    destroy(integrationID) {
        return unpackData(this._client.authApiClient().delete(`/scm_integrations/${integrationID}`));
    }
    /**
    ...
    importSSHKeys(integrationID) {
        return unpackData(this._client
            .authApiClient()
            .post(`/scm_integrations/${integrationID}/import_keys`), "keys");
    }
    /**
    ...
    organizations(integrationID) {
        return unpackData(this._client
            .authApiClient()
            .get(`/scm_integrations/${integrationID}/orgs`), "organizations");
    }
    /**
     * Search repositories in an SCM integration
     * @see https://developers.scalingo.com/scm_integrations#
     ...
     * @return Promise that when resolved returns the list of repositories matching the query.
     */
    branchesForRepo(integrationID, repo_name) {
        return unpackData(this._client
            .authApiClient()
