...
export default class Apps {
    ...
    rename(appID, currentName, newName) {
        return unpackData(this._client.apiClient().post(`/apps/${appID}/rename`, {
     ...
     */
    transfer(appID, currentName, ownerID) {
        return unpackData(this._client.apiClient().patch(`/apps/${appID}`, {
            app: {
                owner: ownerID,
            },
        }, { params: { current_name: currentName } }), "app");
    }
    /**
    ...
    update(appID, appSettings) {
        return unpackData(this._client.apiClient().patch(`/apps/${appID}`, { app: appSettings }), "app");
    }
}
