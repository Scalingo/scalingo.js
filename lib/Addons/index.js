...
export default class Addons {
     ...
     */
    listProviders(categoryId, authenticated = false) {
    ...
    update(appId, addonId, addon) {
        return unpackData(this._client
            .apiClient()
            .patch(`/apps/${appId}/addons/${addonId}`, { addon: addon }), "addon");
    }
    /**
     * Resume an addon
     * @param appId ID of the current application
     * @param addonId ID of the current addon
     */
    resume(appId, addonId) {
        return unpackData(this._client.apiClient().post(`/apps/${appId}/addons/${addonId}/resume`));
    }
    /**
     * Remove an addon
